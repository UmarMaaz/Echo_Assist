...
export default function App() {
  ...
  const unlockAudio = () => {
    const audioContent = "data:audio/mp3;base64,";
    ...
    if (playPromise !== undefined) {
      playPromise.then(_ => {
        console.log("Audio playback started (Unlock success)");
        // Now also try to prime TTS silently just in case
        if ('speechSynthesis' in window) {
  ...
  const processAudioQueue = () => {
    if (audioQueueRef.current.length > 0 && !isPlayingAudioRef.current) {
      ...
      if (audioUrl) {
        ...
        audio.src = audioUrl;
        audio.load(); // Required on some iOS versions when switching src

        const cleanup = () => {
          isPlayingAudioRef.current = false;
          // Revoke URL to free memory on mobile
        ...
        audio.onerror = (e) => {
          ...
          console.log(`Audio error code: ${errCode} (1=Aborted, 2=Network, 3=Decode, 4=SourceNotSupported)`);
          cleanup();
        };
        ...
        if (playPromise !== undefined) {
          playPromise.catch(e => {
            console.error("Play prevented (Autoplay policy?):", e);
            cleanup();
          });
        }
      } else {
